...
2. Notice that the output format defaults directly to **CMake**.
...
1. Open `main.c` and scroll down to the infinite loop block `while(1)`.
2. Notice that by utilizing the software labels configured in STM32CubeMX2, you can interface with pins cleanly without hardcoding physical GPIO ports and pin numbers. 
3. Copy and paste the following code inside the while(1) loop:
...
HAL_Delay(500);
```
4. Compile your application by pressing the build button on the status bar layout (bottom left).
...
<!--
3. Inside your newly generated JSON parameter layout file, type `"` inside the main configuration block array to trigger the autocomplete engine recommendations list.
4. Select and add the following operational argument property parameter:
   ```json
   "serverReset": "Connect under reset",
   ```
-->
3. Click on **Start Debugging (F5)** to download the firmware directly into the physical target STM32C5 MCU and launch a debug session.
4. Click on **Continue (F5)** to resume execution, and observe the LED blinking.
<!--
